...
public class GlobalUpdateService extends IntentService {
	...
	protected void onHandleIntent(Intent intent) {
		...
		if (log.getCount() < 2) {
			Log.v(TAG,"not enough datapoints in the db, return");
			return;
		}
		
		// login to commonsense
		Log.v(TAG,"try to login...");
		CommonSenseAdapter cs = new CommonSenseAdapter(this);
		...
		if (sensorPresent) {
			...
			try {
				...
				JSONObject response = cs.fetchBeaconData();
				...
				if (response==null){						
					...
					newTotalTime = lastUpdateTotalTime+computeIncrement(lastUpdateStatus, lastUpdateTs );							
				}				
	...
	public long computeIncrement(boolean initStatus, long lastUpdateTs) {
		...
		log.moveToFirst();
		Log.d(TAG, "Count: "+log.getCount()+" position: "+log.getPosition());
